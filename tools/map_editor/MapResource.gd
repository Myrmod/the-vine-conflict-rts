...
# Entity placements
@export var placed_structures: Array[Dictionary] = []
...
func _remove_out_of_bounds_placements():
	placed_structures = placed_structures.filter(func(s): return _is_in_bounds(s.pos))
...
func add_structure(scene_path: String, grid_pos: Vector2i, player_id: int, rotation: float = 0.0):
	"""Add a structure placement"""
...
func add_resource_node(scene_path: String, grid_pos: Vector2i, resource_type: String = "resource_a"):
...
func validate() -> Array[String]:
	...
	for structure in placed_structures:
		if not _is_in_bounds(structure.pos):
			errors.append("Structure at %s is outside map bounds" % structure.pos)
