# Factions Overview

Four factions compete over a world overgrown by the Vines.

| Faction | Type | Playstyle |
|---|---|---|
| [The Amuns](Amuns/The_Amuns.md) | Alien architects | Elite, advanced tech, dimensional anchors |
| [The Radix](Radix/The_Radix.md) | Sentient vines | Creep-based, bio-economy, no power grid |
| [The Legion](The_Legion/The_Legion.md) | Human military | Conventional, power-dependent, air power |
| [The Remnants](Remnants/The_Remnants.md) | Human survivors | Cheap, fast, scrappy infantry |

---

## Where to find information

Each faction folder contains a single overview file with:
- **Lore / Origins** — backstory and motivation
- **Economy** — how they harvest Vine resources
- **Tech Tree** — Tier 1 build order
- **Buildings** — costs and stats
- **Units** — stats and abilities

### Shared lore files
- [The Vines — Origins](../The_vines/Origins.md) — what the Vines are and why they exist
- [The Human Conflict — Origins](Human_conflict/Origins.md) — the Ash War and the awakening that split humanity
- [Economy — General](../Economy/General.md) — resource field sizes and harvester mechanics
- [Tech Tree diagrams](../Tech_tree/) — visual .svg / .drawio files per faction

### Scattered source files (superseded by consolidated files above)
| Old file | Covered in |
|---|---|
| `Buildings/Amuns.md` | `Amuns/The_Amuns.md` |
| `Buildings/Radix.md` | `Radix/The_Radix.md` |
| `Buildings/Legion.md` | `The_Legion/The_Legion.md` |
| `Buildings/Remnants.md` | `Remnants/The_Remnants.md` |
| `Units/The_Amuns.md` | `Amuns/The_Amuns.md` |
| `Units/The_Radix.md` | `Radix/The_Radix.md` |
| `Units/The_Legion.md` | `The_Legion/The_Legion.md` |
| `Units/The_Remnants.md` | `Remnants/The_Remnants.md` |
| `Factions/Tech/Tech.md` | All four faction files |
